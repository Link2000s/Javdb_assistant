// ==UserScript==
// @name         Javdb全能助手
// @name:en      Javdb_assistant
// @namespace    https://github.com/Link2000s/Javdb_assistant
// @version      1.0.0
...
(function() {
    ...
    if (window.location.host.includes('sehuatang.net')) {
        ...
        // sehuatang 逻辑结束后仅跳过 JAVDB 初始化，不跳过脚本外层函数
        // 但为了避免后面的 JAVDB 代码在 sehuatang 上报错，直接 return 整个 IIFE
        return;
    }
...
// 原生 JS 版多站点在线观看面板（替换 Preact 组件）
    function addOnlineWatchPanel() {
        if (!window.location.pathname.startsWith('/v/')) return;
        if (document.querySelector('.jop-app')) return;

        // 注入 CSS
        const jopCSS = '.jop-list{box-sizing:border-box;display:flex;flex-wrap:wrap;justify-content:flex-start;gap:10px;width:100%;z-index:1;color:#000}.jop-button,.jop-button_def{position:relative;display:flex;align-items:center;justify-content:center;box-sizing:border-box;padding:3px 10px;border-radius:4px;font-weight:500;font-size:14px;border:1px solid #dcdfe6;color:#606266;cursor:pointer}.jop-button_def{margin:10px 0;width:100px}.jop-button_green{color:#fff!important;background-color:#67c23a}.jop-button_red{color:#fff!important;background-color:#f56c6c}.jop-button_label{position:absolute;font-size:10px;padding:4px;border-radius:4px;top:-13px;right:-10px;line-height:.75;color:#67c23a;border:1px solid #e1f3d8;background:#fff}.jop-setting{margin-top:20px}.jop-setting-list{display:flex;flex-wrap:wrap}.jop-setting-title{margin:10px 0 5px;font-weight:700}.jop-setting-item{display:flex;height:20px;align-items:center;margin-right:15px;cursor:pointer}.db-panel .movie-panel-info div.panel-block{padding:5.5px 12px}.db-panel .jop-app{padding:15px 12px}';
        if (typeof GM_addStyle == 'function') { GM_addStyle(jopCSS); }
        else { const s = document.createElement('style'); s.textContent = jopCSS; document.head.append(s); }

        // 站点列表数据（27个站点）
        const siteList = [
            {name:"FANZA 動画", hostname:"dmm.co.jp", url:"https://www.dmm.co.jp/digital/videoa/-/detail/=/cid={{code}}/", fetchType:"get", codeFormater:(c)=>{const[p,n]=c.split("-");return p.toLowerCase().startsWith("start")?`1${p.toLowerCase()}${n.padStart(5,"0")}`:`${p}${n.padStart(5,"0")}`}},
            {name:"Jable", hostname:"jable.tv", url:"https://jable.tv/videos/{{code}}/", fetchType:"get"},
            {name:"MISSAV", hostname:"missav.ws", url:"https://missav.ws/{{code}}/", fetchType:"get"},
            {name:"123av", hostname:"123av.com", url:"https://123av.com/zh/search?keyword={{code}}", fetchType:"parser"},
            {name:"Supjav", hostname:"supjav.com", url:"https://supjav.com/zh/?s={{code}}", fetchType:"parser"},
            {name:"NETFLAV", hostname:"netflav5.com", url:"https://netflav5.com/search?type=title&keyword={{code}}", fetchType:"parser"},
            {name:"Avgle", hostname:"avgle.com", url:"https://avgle.com/search/videos?search_query={{code}}&search_type=videos", fetchType:"parser"},
            {name:"JAVHHH", hostname:"javhhh.com", url:"https://javhhh.com/v/?wd={{code}}", fetchType:"parser"},
            {name:"BestJP", hostname:"bestjavporn.com", url:"https://www3.bestjavporn.com/search/{{code}}", fetchType:"parser"},
            {name:"JAVMENU", hostname:"javmenu.com", url:"https://javmenu.com/{{code}}", fetchType:"get"},
            {name:"Jav.Guru", hostname:"jav.guru", url:"https://jav.guru/?s={{code}}", fetchType:"parser"},
            {name:"JAVMOST", hostname:"javmost.cx", url:"https://javmost.cx/search/{{code}}/", fetchType:"parser"},
            {name:"HAYAV", hostname:"hayav.com", url:"https://hayav.com/video/{{code}}/", fetchType:"get"},
            {name:"AvJoy", hostname:"avjoy.me", url:"https://avjoy.me/search/videos/{{code}}", fetchType:"parser"},
            {name:"JAVFC2", hostname:"javfc2.net", url:"https://javfc2.net/?s={{code}}", fetchType:"parser"},
            {name:"baihuse", hostname:"paipancon.com", url:"https://paipancon.com/search/{{code}}", fetchType:"parser"},
            {name:"GGJAV", hostname:"ggjav.com", url:"https://ggjav.com/main/search?string={{code}}", fetchType:"parser"},
            {name:"AV01", hostname:"www.av01.tv", url:"https://www.av01.tv/search/videos?search_query={{code}}", fetchType:"parser"},
            {name:"18sex", hostname:"18sex.org", url:"https://www.18sex.org/cn/search/{{code}}/", fetchType:"parser"},
            {name:"highporn", hostname:"highporn.net", url:"https://highporn.net/search/videos?search_query={{code}}", fetchType:"parser"},
            {name:"evojav", hostname:"evojav.pro", url:"https://evojav.pro/video/{{code}}/", fetchType:"get"},
            {name:"18av", hostname:"18av.mm-cg.com", url:"https://18av.mm-cg.com/zh/fc_search/all/{{code}}/1.html", fetchType:"parser"},
            {name:"javgo", hostname:"javgo.to", url:"https://javgo.to/zh/v/{{code}}", fetchType:"get"},
            {name:"javhub", hostname:"javhub.net", url:"https://javhub.net/search/{{code}}", fetchType:"parser"},
            {name:"JavBus", hostname:"javbus.com", url:"https://javbus.com/{{code}}", fetchType:"get"},
            {name:"JavDB", hostname:"javdb.com", url:"https://javdb.com/search?q={{code}}&f=all", fetchType:"get"},
            {name:"JAVLib", hostname:"javlibrary.com", url:"https://www.javlibrary.com/cn/vl_searchbyid.php?keyword={{code}}", fetchType:"get"}
        ];

        // 提取番号
        function getCode() {
            const clip = document.querySelector('[data-clipboard-text]');
            if (clip) return clip.getAttribute('data-clipboard-text');
            const val = document.querySelector('.movie-panel-info .panel-block .value, .movie-panel-info .value');
            if (val) return val.textContent.trim();
            const t = document.querySelector('.title');
            if (t) { const m = t.textContent.trim().match(/([A-Z0-9]{2,12}[-_][A-Z0-9]{2,10})/i); if (m) return m[1].toUpperCase(); }
            return '';
        }
        const CODE = getCode();
        if (!CODE) return;
        ...
        // 面板容器
        const cv = document.querySelector('.column-video-cover');
        if (cv) cv.style.width = '60%';
        ...
        // 设置数据
        const DEF_DIS = ['AvJoy','baihuse','GGJAV','AV01','18sex','highporn','evojav','HAYAV','JavBus','JavDB','JAVLib','MISSAV_','123av','javhub','javgo','JAVMENU'];
        let disables = []; try { disables = JSON.parse(GM_getValue('disable', JSON.stringify(DEF_DIS))); } catch(e) { disables = [...DEF_DIS]; }

        // 渲染按钮列表
        function renderButtons() {
            const list = siteList.filter(s => !disables.includes(s.name));
            const div = document.createElement('div');
            div.className = 'jop-list';
            list.forEach(site => {
                const fc = site.codeFormater ? site.codeFormater(CODE) : CODE;
                const a = document.createElement('a');
                a.className = 'jop-button';
                a.target = '_blank';
                a.href = site.url.replace('{{code}}', fc);
        ...
        function renderSettings() {
            ...
            sd.className = 'jop-setting';
            const t = document.createElement('div');
            t.className = 'jop-setting-title';
            t.textContent = '隐藏站点（点击切换）';
            sd.appendChild(t);
            const ld = document.createElement('div');
            ld.className = 'jop-setting-list';
            siteList.forEach(site => {
                const lb = document.createElement('label');
                lb.className = 'jop-setting-item';
                const cb = document.createElement('input');
                cb.type = 'checkbox';
                cb.checked = disables.includes(site.name);
                cb.onchange = () => {
                    if (cb.checked) { if (!disables.includes(site.name)) disables.push(site.name); }
                    else { disables = disables.filter(d => d !== site.name); }
                    GM_setValue('disable', JSON.stringify(disables));
                    const old = app.querySelector('.jop-list');
                    if (old) { app.replaceChild(renderButtons(), old); }
                };
                lb.appendChild(cb);
                const sp = document.createElement('span');
                sp.textContent = site.name;
        ...
        app.appendChild(renderButtons());
        app.appendChild(renderSettings());
        console.log('JavdbBuddy: 在线观看面板已添加（原生JS），番号:', CODE);
    }
    }
